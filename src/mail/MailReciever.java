...
public class MailReciever {
    ...
    public static void main(String[] args) throws UnknownHostException, IOException {
        ...
        String user = "caoyupersonal@163.com";//用户名，填写自己的邮箱用户名
        String password = "caoyu206590";//密码，填写自己的密码
        MailReciever mailReciever = new MailReciever(server, 110);
        mailReciever.listMail(user,password);
        mailReciever.retrieveMail(3,user, password);
//        mailReciever.listMail(user, password);
    }

    /*构造函数*/
    public MailReciever(String server, int port) throws UnknownHostException, IOException {
        try {
    ...
    //获取邮件列表
    public boolean listMail(String user, String password) {
        try {
...
//            System.out.println("quit 命令执行完毕！");
    ...
    //接收第n封邮件程序
    public boolean retrieveMail(int mailNum, String user, String password) {
        try {
            BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            BufferedWriter out = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()));
//            user(user, in, out);//输入用户名
